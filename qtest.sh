...
./aggregate.py ${args[@]} -r testinput/baseline && diff -qs testinput/golden/golden_QTEST.csv output/QTEST.csv && diff -qs testinput/golden/golden_views_QTEST.csv output/views_QTEST.csv || { echo FAIL; exit 1; }
...
./aggregate.py ${args[@]} -r testinput/repeat_row_tranche && diff -qs testinput/golden/golden_2_QTEST.csv output/QTEST.csv && diff -qs testinput/golden/golden_views_QTEST.csv output/views_QTEST.csv || { echo FAIL; exit 1; }
...
./aggregate.py ${args[@]} -r testinput/additional_row_tranche && diff -qs testinput/golden/golden_2_QTEST.csv output/QTEST.csv && diff -qs testinput/golden/golden_views_2_QTEST.csv output/views_QTEST.csv || { echo FAIL; exit 1; }
...
rm -f output/{views_,}QTEST.csv
./aggregate.py ${page_args[@]} -r testinput/baseline && diff -qs testinput/golden/golden_3_QTEST.csv output/QTEST.csv && diff -qs testinput/golden/golden_views_3_QTEST.csv output/views_QTEST.csv || { echo FAIL; exit 1; }

#Continue by making one page almost, but not quite, complete. Output should be the same.
rm -f output/QTEST.csv #Deliberately keep the previous views file
./aggregate.py ${page_args[@]} -r testinput/incomplete_page_tranche && diff -qs testinput/golden/golden_3_QTEST.csv output/QTEST.csv && diff -qs testinput/golden/golden_views_3_QTEST.csv output/views_QTEST.csv || { echo FAIL; exit 1; }

#Then complete the page. Now we should get it in the output and in the views file.
rm -f output/QTEST.csv #Deliberately keep the previous views file
./aggregate.py ${page_args[@]} -r testinput/complete_page_tranche && diff -qs testinput/golden/golden_4_QTEST.csv output/QTEST.csv && diff -qs testinput/golden/golden_views_4_QTEST.csv output/views_QTEST.csv || { echo FAIL; exit 1; }

#Repeat, with the effect of that views file simulated. views file should be unchanged, output should be empty again.
rm -f output/QTEST.csv #Deliberately keep the previous views file
./aggregate.py ${page_args[@]} -r testinput/repeat_complete_page_tranche && diff -qs testinput/golden/golden_3_QTEST.csv output/QTEST.csv && diff -qs testinput/golden/golden_views_4_QTEST.csv output/views_QTEST.csv || { echo FAIL; exit 1; }

#Complete one more page. Both outputs should now contain that page.
rm -f output/QTEST.csv #Deliberately keep the previous views file
./aggregate.py ${page_args[@]} -r testinput/additional_page_tranche && diff -qs testinput/golden/golden_5_QTEST.csv output/QTEST.csv && diff -qs testinput/golden/golden_views_5_QTEST.csv output/views_QTEST.csv || { echo FAIL; exit 1; }
