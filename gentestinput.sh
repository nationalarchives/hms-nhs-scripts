...
for x in aggregation/text_reducer_*.csv; do
...
for x in aggregation/dropdown_reducer_*.csv; do
  csvtool namedcol subject_id,task,data.value $x > testinput/`basename $x`
done

ln aggregation/Task_labels_workflow_186{1,2}4_V3.1.yaml testinput/
